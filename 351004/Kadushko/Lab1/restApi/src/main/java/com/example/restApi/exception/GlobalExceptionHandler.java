...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorInfo> handleAlreadyExistsException(AlreadyExistsException ex) {
        ErrorInfo error = new ErrorInfo(ex.getMessage(), 40301);
    ...
    public ResponseEntity<ErrorInfo> handleIllegalArgumentException(IllegalArgumentException ex) {
        ErrorInfo error = new ErrorInfo(ex.getMessage(), 40002);
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(error);
    }
