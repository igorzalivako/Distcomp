...
public class ReactionService {
    ...
    public ReactionResponseTo create(ReactionRequestTo dto) {
        if (!tweetRepository.existsById(dto.getTweetId())) {
            throw new ResourceNotFoundException("Tweet with id " + dto.getTweetId() + " not found", 40416);
        }
    ...
    public ReactionResponseTo getById(Long id) {
        Reaction entity = reactionRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Reaction with id " + id + " not found", 40417));
    ...
    public ReactionResponseTo update(ReactionRequestTo dto) {
        Reaction existingReaction = reactionRepository.findById(dto.getId())
                .orElseThrow(() -> new ResourceNotFoundException("Reaction with id " + dto.getId() + " not found", 40418));
        
        if (!tweetRepository.existsById(dto.getTweetId())) {
            throw new ResourceNotFoundException("Tweet with id " + dto.getTweetId() + " not found", 40419);
        }
    ...
    public void deleteById(Long id) {
        if (!reactionRepository.existsById(id)) {
            throw new ResourceNotFoundException("Reaction with id " + id + " not found", 40420);
        }
        reactionRepository.deleteById(id);
    }

    public List<ReactionResponseTo> getReactionsByTweetId(Long tweetId) {
        if (!tweetRepository.existsById(tweetId)) {
            throw new ResourceNotFoundException("Tweet with id " + tweetId + " not found", 40421);
        }
        
        return reactionRepository.findByTweetId(tweetId).stream()
                .map(reactionMapper::toResponseDto)
                .collect(Collectors.toList());
    }
}
